import { ActionTypes, Actions, AddToCartType, DefaultCartDataStateType, RemoveItemFromCartType } from './types'
...
export const cartData = (state = defaultCartData, action: Actions): DefaultCartDataStateType => {
  switch (action.type) {
    case ActionTypes.addToCart:
      console.log('ADD_TO_CART condition ', action)
      const itemToAdd = (action as AddToCartType).payload.data
      const existingItemIndex = state.items.findIndex((entry) => entry.item.id === itemToAdd.id)
    ...
    case ActionTypes.removeItemFromCart:
      ...
      const indexToRemove = state.items.findIndex((entry) =>
